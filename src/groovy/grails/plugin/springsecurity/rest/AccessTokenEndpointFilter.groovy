 ...
 */
@Slf4j
class AccessTokenEndpointFilter extends GenericFilterBean {
    ...
    void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if (actualUri == endpointUrl) {
            ...
            if (httpServletRequest.getParameter('grant_type') == 'refresh_token') {
                authenticationResult = restore(httpServletRequest, httpServletResponse)
            } else {
                authenticationResult = authenticate(httpServletRequest, httpServletResponse)
            }
    ...
    Authentication restore(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse){
        String refreshToken = httpServletRequest.getParameter('refresh_token')
        log.debug "Trying to generate an access token for the refresh token: ${refreshToken}"
        if (refreshToken) {
            try {
                def principal = tokenStorageService.loadUserByToken(refreshToken) as User
